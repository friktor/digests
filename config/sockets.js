...
module.exports.sockets = {
 ...
 /* onConnect: function (session, socket) {
    if (session.auth && session.user && session.user.online === false) {
      sails.log.info("User is logged in, but not set value online is true");

      User.findOneByUsername(session.user.username, function (error, user) {
        if (user) {
          user.online = true;
          user.save();
        };
  ...
  afterDisconnect: function (session, socket, next) {
    if (session.auth === true && session.user.username) {
      sails.log.info("User has been disconnect");
